...
public class FIFOCorridor implements IFIFOCorridor {
    ...
    /**
     * Function that lets the costumer do a step
     * @param customerId Id of the costumer     
     * @param nTimesWalked Number of times the costumer already did a step
     * @param fifoCorridorHall costumer when leaving, calls another costumer from the corridorHall
     * @return 
     */
    @Override
    public int inCostumer(int customerId, int nTimesWalked, FIFOCorridor fifoCorridorHall) {
        ...
        try {
            ...
            if (this.customerId[9] == customerId || stuck8 == 1) {
                if (stuck8 == 1) {
                    while (!leave[8]) // qd se faz await, permite-se q outros thread tenham acesso
                    ...
                    while (!leave[9]) // qd se faz await, permite-se q outros thread tenham acesso
                    // à zona protegida pelo lock
                    {
                        cStay[9].await();
                    }
    ...
    /**
     * Function to let costumers go inside CorridorHalls
     * @param customerId id of the costumer
     * @param nextCountCorridor the count of the corridor that is connected to this corridorhall
     * @param firstSlotOpen check if the first step of the corridor is empty
     * @return 
     */
    @Override
    public int in(int customerId, int nextCountCorridor, boolean firstSlotOpen) {
        ...
        try {
            ...
            if (nextCountCorridor >= 1 || firstSlotOpen == false) {
                while (!leave[idx]) // qd se faz await, permite-se q outros thread tenham acesso
                // à zona protegida pelo lock
                {
                    cStay[idx].await();
                }
    ...
    /**
     * function that calls a costumer that is waiting on the last step of the corridor and if there is another costumer that is waiting on the step before, wakes him up
     */
    @Override
    public void out() {
        try {
            ...
            while (leave[9] == true) // qd se faz await, permite-se q outros thread tenham acesso
            // à zona protegida pelo lock
            {
                cLeaving.await();
            }
    ...
    /**
     * Function that calls a costumer from the corridorHall
     */
    @Override
    public void outCostumer() {
        try {
            ...
            cStay[idx].signal();
            // aguardar até q Customer saia do fifo
            while (leave[idx] == true) // qd se faz await, permite-se q outros thread tenham acesso
            // à zona protegida pelo lock
            {
                cLeaving.await();
            }
